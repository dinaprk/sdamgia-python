...
class SdamGIA:
    ...
    def get_problem_by_id(self, subject, id, img=None, path_to_img=None, path_to_html='', grabzit_auth=None):
        ...
        condition_html = str(probBlock.find_all('div', class_="pbody")[0])
        solution_html = str(probBlock.find_all('div', class_="pbody")[1])
        print(probBlock)
        ...
        if not img is None:
            ...
            if img == 'pyppeteer':
                ...
                async def take_screenshot():
                    ...
                    print('file:' + path.abspath(f'{path_to_html}{id}.html'))
    ...
    def get_category_problems(self, subject, categoty_id):
        response = requests.get(f'{self._SUBJECT_BASE_URL[subject]}/test?filter=all&category_id={categoty_id}')
    ...
    def generate_pdf(self, subject: str, testid: str, solution='', nums='',
        ...
        return self._SUBJECT_BASE_URL[subject] + requests.get(f'{self._SUBJECT_BASE_URL[subject]}/test', params=params,
...
if __name__ == '__main__':
    sdamgia = SdamGIA()
    data = sdamgia.get_problem_by_id('math', '351890')
    print(json.dumps(data, indent=4, ensure_ascii=False))
    # with open()

    # pdf_mode = 'z'
    # print(pdf_mode, ": ", sdamgia.generate_pdf('math', '53499670', solution=True, key=True,  nums=True, pdf=pdf_mode,
    #                                            tt='Разбор всех задач 25-ого типа из ОГЭ'))
